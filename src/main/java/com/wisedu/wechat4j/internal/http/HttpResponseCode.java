...
public interface HttpResponseCode {
    ...
    int INTERNAL_SERVER_ERROR = 500;// Internal Server Error: Something is broken. Please post to the group so the wechat team can investigate.
    int BAD_GATEWAY = 502;// Bad Gateway: Wechat is down or being upgraded.
    int SERVICE_UNAVAILABLE = 503;// Service Unavailable: The Wechat servers are up, but overloaded with requests. Try again later. The search and trend methods use this to indicate when you are being rate limited.
    int GATEWAY_TIMEOUT = 504;// The Wechat servers are up, but the request couldn't be serviced due to some failure within our stack. Try again later.
}
